<template>
  <a-modal title="选择位置" :width="900" :visible="visible" :confirmLoading="confirmLoading" @ok="handleSubmit" @cancel="handleCancel">
    <!-- 地图 -->
    <div id="map1" class="web-map">
      <!-- 坐标显示 -->
      <div class="local-inline">
        <a-input :value="longitude" style="width: 130px;margin-right: 12px;" placeholder="经度" />
        <a-input :value="latitude" style="width: 130px" placeholder="纬度" />
      </div>
    </div>
  </a-modal>
</template>
...
export default {
  data () {
    return {
      ...
      longitude: 129.509089271,
      latitude: 42.891117162
    }
  },
  methods: {
    // 初始化方法
    ...
    */
    getLngLat () {
      ...
      this.mapView.on('click', function (e) {
        that.$message.success('当前坐标已成功获取到！')
