...
<script>
  ...
  import { detectZyEdit } from '@/api/modular/main/detectzy/detectZyManage'
  export default {
    data () {
      return {
        labelCol: {
      ...
      edit (record) {
        ...
        setTimeout(() => {
          this.form.setFieldsValue(
            {
              pipeconduitid: record.pipeconduitid,
              detectUnit: record.detectUnit,
              detectPerson: record.detectPerson,
              contacts: record.contacts,
              detectMethod: (record.detectMethod == null || record.detectMethod === '') ? undefined : String(record.detectMethod),
              detectDir: (record.detectDir == null || record.detectDir === '') ? undefined : String(record.detectDir),
              pipeBlock: record.pipeBlock,
              siltThickness: record.siltThickness,
              funeDefect: (record.funeDefect == null || record.funeDefect === '') ? undefined : String(record.funeDefect),
              funeLength: record.funeLength,
              funeAddr: record.funeAddr,
              funeClass: record.funeClass,
              structDefect: (record.structDefect == null || record.structDefect === '') ? undefined : String(record.structDefect),
              structLength: record.structLength,
              structAddr: record.structAddr,
              structClass: record.structClass,
              repairIndex: record.repairIndex,
              maintainIndex: record.maintainIndex,
        ...
        // 时间单独处理
        if (record.detectTime) {
            this.form.getFieldDecorator('detectTime', { initialValue: moment(record.detectTime, 'YYYY-MM-DD') })
            this.detectTimeDateString = moment(record.detectTime).format('YYYY-MM-DD')
        }
        // 时间单独处理
