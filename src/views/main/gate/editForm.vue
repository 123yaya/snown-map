...
<script>
  ...
  import { gateEdit } from '@/api/modular/main/gate/gateManage'
  export default {
    data () {
      return {
        labelCol: {
          ...
          this.form.setFieldsValue(
            {
              ...
              ycoor: record.ycoor,
              name: record.name,
              gateCategory: (record.gateCategory == null || record.gateCategory === '') ? undefined : String(record.gateCategory),
              manufacturer: record.manufacturer,
              model: record.model,
              topElev: record.topElev,
              ...
              controlType: (record.controlType == null || record.controlType === '') ? undefined : String(record.controlType),
              switchStyle: (record.switchStyle == null || record.switchStyle === '') ? undefined : String(record.switchStyle),
              rowNum: record.rowNum,
              holeNum: record.holeNum,
      ...
      handleSubmit () {
        ...
        validateFields((errors, values) => {
          if (!errors) {
            ...
            values.gateid = this.id
            gateEdit(values).then((res) => {
              if (res.success) {
