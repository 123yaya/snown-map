...
<script>
  import {
    emergencyPlanDetail,
    emergencyPlanEdit,
    emergencyPlanUploadFile
  } from '@/api/modular/main/emergencyplan/emergencyPlanManage'
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        this.visible = true
        this.id = record.id
        this.levelSelected = record.level
              ...
              this.form.setFieldsValue(
                {
                  ...
                  notePersonIds: JSON.parse(res.data.notePersonIds)
                }
              )

              if (res.data.emergencyPlanFiles && res.data.emergencyPlanFiles.length > 0) {
                const emergencyPlanFiles = res.data.emergencyPlanFiles
                for (let i = 0; i < emergencyPlanFiles.length; i++) {
                  this.uploadFiles.push({
                    name: emergencyPlanFiles[i].fileOriginName,
                    status: 'done',
                    uid: emergencyPlanFiles[i].id,
                    response: {
                      id: emergencyPlanFiles[i].id
                    }
                  })
                }
              }
      ...
      handleSubmit () {
        ...
        validateFields((errors, values) => {
          if (!errors) {
            ...
            emergencyPlanEdit(values).then((res) => {
              if (res.success) {
