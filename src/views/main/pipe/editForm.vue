...
<script>
  ...
  import { pipeEdit } from '@/api/modular/main/pipe/pipeManage'
  export default {
    data () {
      return {
        labelCol: {
    ...
    methods: {
      ...
      edit (record) {
        this.visible = true
        this.id = record.pipeid
        this.sysDictTypeDropDown()
        setTimeout(() => {
          this.form.setFieldsValue(
            {
              systemid: record.systemid,
              pipeCategory: (record.pipeCategory == null || record.pipeCategory === '') ? undefined : String(record.pipeCategory),
              ...
              invertsilphon: (record.invertsilphon == null || record.invertsilphon === '') ? undefined : String(record.invertsilphon),
              shapeData1: record.shapeData1,
              shaneData2: record.shaneData2,
              shaneData3: record.shaneData3,
              shaneData4: record.shaneData4,
              shaneXydata: record.shaneXydata,
              ...
              pressureType: (record.pressureType == null || record.pressureType === '') ? undefined : String(record.pressureType),
              wallThickness: record.wallThickness,
              linerMaterial: (record.linerMaterial == null || record.linerMaterial === '') ? undefined : String(record.linerMaterial),
              linerThickness: record.linerThickness,
              ...
              pipeSlope: record.pipeSlope,
              ownership: (record.ownership == null || record.ownership === '') ? undefined : String(record.ownership),
              shapeArea: record.shapeArea,
              selfwashing: (record.selfwashing == null || record.selfwashing === '') ? undefined : String(record.selfwashing),
              rainsewamix: (record.rainsewamix == null || record.rainsewamix === '') ? undefined : String(record.rainsewamix),
              originstruct: (record.originstruct == null || record.originstruct === '') ? undefined : String(record.originstruct),
              constrMethod: (record.constrMethod == null || record.constrMethod === '') ? undefined : String(record.constrMethod),
              trenchlessMethod: (record.trenchlessMethod == null || record.trenchlessMethod === '') ? undefined : String(record.trenchlessMethod),
      ...
      handleSubmit () {
        ...
        validateFields((errors, values) => {
          if (!errors) {
            ...
            values.pipeid = this.id
            pipeEdit(values).then((res) => {
              if (res.success) {
